...
const MyTeamsAgents = () => {
  const { agents, voteAgent, getVotes, toggleFavorite, isFavorite } = useAgents();
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('All');

  // Get all shared agents from team members (both certified and uncertified)
  const teamAgents = agents.filter(agent => agent.isShared);
  ...
  // Filter and sort by usage to get the true ranking
  const rankedAgents = [...teamAgents]
    .filter(agent => {
            ...
            <div className="space-y-3">
              {filteredAgents.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  No team agents found matching your criteria
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <Badge className={`${getStatusColor(agent.status)} justify-center`}>
                        {agent.status === 'pending' ? 'Pending' : agent.status}
                      </Badge>
                    </Link>
                  );
                })
